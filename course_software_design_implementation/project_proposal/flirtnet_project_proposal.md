# FlirtNet Project Proposal

## 1. Introduction and Project Idea

FlirtNet is a supportive social discovery platform designed to help users throughout the full interaction journey, rather than only helping them get a match. Many existing dating or social discovery platforms focus heavily on first impressions, quick matching, and short-term engagement. However, users often need more support after a match is made. They may not know how to start a conversation, how to keep it going, how to decide whether they want a more meaningful relationship, or how to deal with negative experiences such as harassment or breakup.

Our proposed project aims to build a more complete and caring platform for online social interaction. FlirtNet combines user discovery, real-time chat, datemate relationship development, community interaction, safety support, and emotional support into one system. Instead of treating social interaction as a short and isolated event, the platform is designed to support the whole process, from meeting someone for the first time to developing a datemate relationship, and even to finding closure when that relationship ends.

The project is especially suitable for a software design and implementation course because it involves multiple connected software modules, a clear need for system architecture, real-time communication, data management, user interaction design, and practical decisions about scalability and maintainability. It is also large enough to justify team-based work, because the system includes account management, profile handling, matching, messaging, safety-related functions, community features, and copilot-based support features.
...
The proposed project will focus on a realistic proof of concept that demonstrates the main design idea of FlirtNet. The project scope is divided into core features for the proof of concept, secondary features that may be added if time allows, and explicit exclusions that will not be part of the initial proof of concept.

### 4.1 Core PoC Features

The core proof of concept will include the following features:
...
- **Profile optimizer** so that users can improve their self-description and express themselves more clearly.

These features represent the main identity of FlirtNet. Together, they form a complete demonstration of the platform’s core idea: helping users meet, communicate, build datemate relationships, stay safe, and receive support when things go wrong.
...
- **Notifications** for messages, datemate requests, and moderation outcomes.
...
- **Native mobile app** as a later platform extension.
...
FlirtNet will be developed as a server-rendered multi-page web application. This means the system will provide separate web pages for the main user workflows, such as registration, profile management, random chat, datemates, private chat, and Community Square. This approach fits the project well because it keeps the application structure clear, supports page-based navigation, and keeps the frontend architecture easier to manage for the project scope.

The backend will be developed in **Go**, and the main database will be **PostgreSQL**. The overall architectural style will be a **modular monolith**. This means the system will run as one main application, but the codebase will be separated into clear functional modules. This approach is appropriate for the project because it provides better structure than a simple monolithic implementation, while remaining more feasible than splitting the system into many separate services too early.
...
The main persistent data store will be PostgreSQL. This is suitable because the platform depends on structured relational data, including users, profiles, datemate relationships, chat records, posts, comments, and reports. Compared with a lightweight local database, PostgreSQL offers a more professional foundation for data consistency, future growth, and more reliable development practice.

The system will also include a real-time communication channel for matching and chat. This is necessary because random matching and live messaging depend on fast and continuous interaction between users. In addition to normal HTTP request and response flow, the system therefore needs a persistent real-time layer for live communication events.

Redis is treated as an optional supporting component. It may be introduced for temporary state, caching, or queue-related functions if the system design benefits from it, especially in features such as matching or session support. However, it is not a required dependency for the initial proof of concept.
...
- a real-time communication channel for matching and live chat
- PostgreSQL as the main persistent storage boundary
- optional Redis support for temporary or performance-related data
...
This methodology also fits the course objectives well. It demonstrates software planning, teamwork, incremental implementation, and practical decision-making instead of relying on a single large development phase near the deadline.
...
- **Early April 2026:** confirm detailed requirements and finalize the architecture
- **Mid April 2026:** set up backend structure, database foundation, and frontend page structure

### 7.3 Core Implementation Stage

- **Late April 2026 to Mid May 2026:** implement core features, including accounts, profiles, matching, chat, datemate workflow, Community Square, and core copilot support

### 7.4 Integration and Refinement Stage

- **Late May 2026:** carry out integration, testing, and refinement of the proof of concept

### 7.5 Submission Preparation Stage

- **Early June 2026:** complete the proof of concept and prepare the user manual
- **Mid June 2026:** prepare the final report and presentation materials

This timeline is realistic because it puts the strongest focus on the core proof of concept features first, while keeping secondary features available only if the main implementation remains stable.
...
- **web templating and frontend tooling** for the multi-page web interface

### 8.2 External Services and Optional Components

The project may also rely on:

- an **external copilot-related API service** for conversation and support features
- **Redis** as an optional component for caching, queueing, or temporary state management

### 8.3 Team Roles

The project will require work in several areas, including project coordination, backend development, frontend development, database work, testing, documentation, and integration. Since some team members may take multiple roles, the detailed role assignment will be finalized separately. However, the proposal assumes that the team structure will support both technical implementation and project management tasks throughout the full development period.
...
From a software engineering perspective, the project is suitable for the course because it requires clear system design, structured implementation, real-time interaction, database-backed features, and teamwork across several connected modules. The selected technical direction, which uses a Go backend, PostgreSQL, a modular monolith architecture, and a server-rendered multi-page web interface, provides a practical and coherent foundation for development.
